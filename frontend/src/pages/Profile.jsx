    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="pt-16 max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          {/* Gradient banner */}
          <div className="h-20 rounded-xl mb-4 relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(255,107,0,0.2), rgba(255,119,0,0.12), rgba(255,173,92,0.08))' }}>
            <div className="absolute inset-0" style={{ backdropFilter: 'blur(4px)' }} />
          </div>
          <div className="flex items-start gap-5 -mt-10 px-1">
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <div className="flex flex-wrap gap-2">
              {user.skillsOffered.map((s, i) => (
                <motion.span key={s.name} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.04 }}
